...
@public 
@payable
def create_campain(_num_coupon: uint256, _wei_per_redeemtion: wei_value, _name: string[50], _category: string[20], _description: string[100], _time_limit: timedelta):
    ...
    new_campain: address = create_forwarder_to(self.campain_template)
    Campain(new_campain).initialize(msg.sender, _num_coupon, _wei_per_redeemtion, _time_limit, _required_value)
    log.NewCampain(msg.sender, _num_coupon, _wei_per_redeemtion, _name, _category, _description)
